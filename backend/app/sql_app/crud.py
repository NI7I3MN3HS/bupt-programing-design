...
def create_private_message(db: Session, private_message: schemas.PrivateMessageCreate):
    db_private_message = models.PrivateMessage(
...
def get_private_message(db: Session, private_message_id: int):
    return (
        ...
        .filter(models.PrivateMessage.id == private_message_id)
        .first()
    )
...
# 获取用户的所有私信
def get_private_messages_by_user(
    db: Session, user_id: int, skip: int = 0, limit: int = 100
):
    return (
        db.query(models.PrivateMessage)
        .filter(models.PrivateMessage.sender_id == user_id)
        .offset(skip)
        .limit(limit)
        .all()
    )
